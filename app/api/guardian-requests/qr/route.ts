...
export async function GET(request:Request) {
  ...
  const row=await database().prepare("SELECT g.*,s.name,s.nis,s.guardian_email,s.guardian_phone FROM guardian_requests g JOIN students s ON s.id=g.student_id WHERE g.id=?").bind(id).first<Record<string,unknown>>();
  if(!row) return Response.json({error:"Permintaan tidak ditemukan."},{status:404});
  if(user.role==="Wali Santri"&&!(await guardianOwnsStudent(user,Number(row.student_id)))) return Response.json({error:"Permintaan tidak terhubung dengan akun ini."},{status:403});
