...
export async function GET(request:Request) {
  try {
    ...
    return Response.json({provider:"firebase",sessions:await listFirebaseSessions(request,session.uid),mfa:{available:true,enrolled:Boolean(account.multiFactor?.enrolledFactors?.length),required:user.role==="Admin"}});
  } catch(error) {
