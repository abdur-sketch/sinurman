...
function Metric({ title, value, icon, tone }: { title:string; value:number|string; icon:string; tone:string }) {
  return <article className="metric-card"><MiniIcon tone={tone}>{icon}</MiniIcon><div><span>{title}</span><strong>{value}</strong><small>Data SPMB terkini</small></div></article>;
}
...
function StudentsPage({ data, editable, onAdd, onEdit, onDelete, onCard }: { data:AppData; editable:boolean; onAdd: () => void; onEdit: (row: Row) => void; onDelete: (row: Row) => void; onCard:(row:Row)=>void }) {
  ...
  if(selected) {
    ...
    return <div className="student-360-page">
      ...
      {detailTab==="akademik"&&<section className="student-360-grid"><article className="card student-360-card report-summary-card"><h3>Ringkasan Rapor Otomatis</h3>{grades.length?<><div className="student-summary-metrics"><div><span>Rata-rata</span><strong>{Math.round(grades.reduce((sum,row)=>sum+Number(row.final_score||0),0)/grades.length)}</strong></div><div><span>Tuntas</span><strong>{grades.filter(row=>Number(row.final_score)>=Number(row.minimum_score||75)).length}/{grades.length}</strong></div><div><span>Kehadiran</span><strong>{attendanceRate}%</strong></div></div><p>{(() => { const avg=Math.round(grades.reduce((sum,row)=>sum+Number(row.final_score||0),0)/grades.length); const passedCount=grades.filter(row=>Number(row.final_score)>=Number(row.minimum_score||75)).length; return `${selected.name} memperoleh rata-rata nilai ${avg}. ${passedCount===grades.length?"Seluruh mata pelajaran telah mencapai KKM.":`Sebanyak ${passedCount} dari ${grades.length} mata pelajaran telah mencapai KKM.`} Tingkat kehadiran tercatat ${attendanceRate}%.`; })()}</p></>:<p className="muted">Ringkasan akan dibuat otomatis setelah nilai akademik dimasukkan.</p>}</article><article className="card student-360-card report-summary-card"><h3>Ringkasan Kesiswaan</h3><div className="student-summary-metrics"><div><span>Poin pembinaan</span><strong>{points>0?`+${points}`:points}</strong></div><div><span>Catatan</span><strong>{counseling.length}</strong></div><div><span>Izin</span><strong>{permits.filter(row=>row.status==="Disetujui").length}</strong></div></div><p>{counseling.length?`Terdapat ${counseling.length} catatan pembinaan yang perlu dipantau oleh pembina.`:"Belum ada catatan pembinaan untuk santri ini."}</p></article></section>}
...
function AcademicPage({ data, role, edit, remove }: { data:AppData; role:Role; edit:(r:Resource,row?:Row)=>void; remove:(r:Resource,row:Row)=>void }) {
  ...
  const classes=[...new Set(data.grades.map(row=>String(row.class_name||"" )).filter(Boolean))];
  const semesters=[...new Set(data.grades.map(row=>String(row.semester||"" )).filter(Boolean))];
  const grades=data.grades.filter(row=>(classFilter==="Semua Kelas"||String(row.class_name)===classFilter)&&(semesterFilter==="Semua Semester"||String(row.semester)===semesterFilter));
  const average=grades.length?Math.round(grades.reduce((sum,row)=>sum+Number(row.final_score||0),0)/grades.length):0;
  const passed=grades.filter(row=>Number(row.final_score)>=Number(row.minimum_score||75)).length;
  const incomplete=data.students.filter(student=>!grades.some(row=>String(row.student_id)===String(student.id))).length;
  return <><section className="stats-grid three"><article className="metric-card"><MiniIcon tone="blue">A</MiniIcon><div><span>Rata-rata Nilai</span><strong>{average||"-"}</strong><small>Bobot 30% tugas, 30% PTS, 40% PAS</small></div></article><article className="metric-card"><MiniIcon tone="green">✓</MiniIcon><div><span>Tuntas</span><strong>{passed}</strong><small>dari {data.grades.length} nilai</small></div></article><article className="metric-card"><MiniIcon tone="violet">▦</MiniIcon><div><span>Mata Pelajaran</span><strong>{data.subjects.length}</strong><small>SMP dan SMK</small></div></article></section>
    <section className="card academic-filters"><div><strong>Filter rapor</strong><small>{grades.length} nilai tampil · {incomplete} santri belum memiliki nilai pada filter ini</small></div><select value={classFilter} onChange={event=>setClassFilter(event.target.value)}><option>Semua Kelas</option>{classes.map(value=><option key={value}>{value}</option>)}</select><select value={semesterFilter} onChange={event=>setSemesterFilter(event.target.value)}><option>Semua Semester</option>{semesters.map(value=><option key={value}>{value}</option>)}</select><a className="secondary-button link-button" href="/cetak?type=academics&auto=1" target="_blank" rel="noreferrer">Cetak Rapor</a></section>
    {!data.subjects.length&&<div className="sync-banner">Master mata pelajaran masih kosong. Tambahkan mata pelajaran terlebih dahulu sebelum input nilai.</div>}
    <section className="dashboard-grid operations-grid"><article className="card data-card"><header className="card-header"><div><h3>Nilai Akademik</h3><p>Nilai akhir dan predikat dihitung otomatis</p></div><button className="primary-button" onClick={()=>edit("grades")}>+ Input Nilai</button></header><div className="table-wrap"><table><thead><tr><th>Santri</th><th>Mata Pelajaran</th><th>Tugas</th><th>PTS</th><th>PAS</th><th>Nilai Akhir</th><th /></tr></thead><tbody>{grades.map(row=><tr key={String(row.id)}><td><strong>{row.student_name}</strong></td><td>{row.subject_name}<small className="cell-note">{row.semester} · {row.academic_year}</small></td><td>{row.assignment_score}</td><td>{row.midterm_score}</td><td>{row.exam_score}</td><td><Status tone={Number(row.final_score)>=Number(row.minimum_score||75)?"green":"amber"}>{row.final_score} · {row.predicate}</Status></td><td><DataActions row={row} onEdit={item=>edit("grades",item)} onDelete={item=>remove("grades",item)}/></td></tr>)}{!grades.length&&<tr><td colSpan={7} className="muted">Belum ada nilai pada filter yang dipilih.</td></tr>}</tbody></table></div></article>
...
function AttendancePage({ data, edit, remove, reload, notify }: { data:AppData; edit:(r:Resource,row?:Row)=>void; remove:(r:Resource,row:Row)=>void; reload:()=>Promise<void>; notify:(message:string)=>void }) {
  ...
  const [offlineQueue,setOfflineQueue]=useState<Array<{token:string;status:string}>>([]);
  const [attendanceDate,setAttendanceDate]=useState("");
  const [attendanceFilter,setAttendanceFilter]=useState("Semua");
  const visibleAttendance=data.attendance.filter(row=>(!attendanceDate||String(row.record_date||"")===attendanceDate)&&(attendanceFilter==="Semua"||String(row.status)===attendanceFilter));
  const dateAttendance=data.attendance.filter(row=>!attendanceDate||String(row.record_date||"")===attendanceDate);
  const attendanceCount=(status:string)=>dateAttendance.filter(row=>String(row.status)===status).length;
  const attendanceRate=dateAttendance.length?Math.round((attendanceCount("Hadir")+attendanceCount("Terlambat"))/dateAttendance.length*100):0;
  useEffect(()=>{try{const queued=JSON.parse(window.localStorage.getItem("sinurman_attendance_queue")||"[]") as Array<{token:string;status:string}>;setOfflineQueue(queued);if(!queued.length)return;void Promise.all(queued.map(item=>fetch("/api/attendance-qr",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({token:item.token,status:item.status})}).then(response=>{if(!response.ok)throw new Error("sync");}))).then(()=>{window.localStorage.removeItem("sinurman_attendance_queue");setOfflineQueue([]);void reload();}).catch(()=>{});}catch{/* browser storage unavailable */}},[reload]);
  async function scanAttendance() {
    setScanning(true);
    let response:Response;
    try { response=await fetch("/api/attendance-qr",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({token:attendanceToken,status:attendanceStatus})}); } catch { const queued=[...offlineQueue,{token:attendanceToken,status:attendanceStatus}]; try{window.localStorage.setItem("sinurman_attendance_queue",JSON.stringify(queued));}catch{/* storage unavailable */} setOfflineQueue(queued);setScanning(false);setAttendanceToken("");notify("Jaringan terputus. Presensi disimpan dan akan disinkronkan otomatis.");return; }
  ...
  return <><section className="stats-grid four"><article className="metric-card"><MiniIcon tone="green">✓</MiniIcon><div><span>Hadir</span><strong>{attendanceCount("Hadir")}</strong><small>{attendanceRate}% efektif</small></div></article><article className="metric-card"><MiniIcon tone="blue">↗</MiniIcon><div><span>Terlambat</span><strong>{attendanceCount("Terlambat")}</strong><small>Perlu pembinaan</small></div></article><article className="metric-card"><MiniIcon tone="amber">◷</MiniIcon><div><span>Izin/Sakit</span><strong>{attendanceCount("Izin")+attendanceCount("Sakit")}</strong><small>{data.permits.filter(x=>x.status==="Disetujui").length} izin disetujui</small></div></article><article className="metric-card"><MiniIcon tone="red">!</MiniIcon><div><span>Alpa</span><strong>{attendanceCount("Alpa")}</strong><small>Perlu ditindaklanjuti</small></div></article></section>
  <section className="card qr-attendance"><MiniIcon tone="blue">QR</MiniIcon><div><h3>Presensi Kartu Santri</h3><p>Scan QR kartu dengan kamera/perangkat pemindai, lalu tempel hasilnya. Satu santri hanya memiliki satu catatan per hari.</p>{offlineQueue.length>0&&<small className="offline-queue-note">{offlineQueue.length} presensi menunggu sinkronisasi</small>}</div><select value={attendanceStatus} onChange={event=>setAttendanceStatus(event.target.value)}><option>Hadir</option><option>Terlambat</option><option>Sakit</option><option>Izin</option><option>Alpa</option></select><input value={attendanceToken} onChange={event=>setAttendanceToken(event.target.value)} placeholder='Hasil scan QR kartu santri' /><button className="primary-button" disabled={scanning||!attendanceToken.trim()} onClick={()=>void scanAttendance()}>{scanning?"Memproses…":"Catat Presensi"}</button></section>
  <section className="dashboard-grid operations-grid"><article className="card data-card"><header className="card-header responsive"><div><h3>Absensi Santri</h3><p>{visibleAttendance.length} catatan sesuai filter · tingkat hadir {attendanceRate}%</p></div><div className="header-actions"><label className="compact-filter">Tanggal<input type="date" value={attendanceDate} onChange={event=>setAttendanceDate(event.target.value)} /></label><label className="compact-filter">Status<select value={attendanceFilter} onChange={event=>setAttendanceFilter(event.target.value)}><option>Semua</option><option>Hadir</option><option>Terlambat</option><option>Sakit</option><option>Izin</option><option>Alpa</option></select></label><button className="secondary-button" onClick={()=>{setAttendanceDate("");setAttendanceFilter("Semua");}}>Reset</button><button className="primary-button" onClick={()=>edit("attendance")}>+ Catat Absensi</button></div></header><div className="table-wrap"><table><thead><tr><th>Santri</th><th>Tanggal</th><th>Status</th><th>Catatan</th><th /></tr></thead><tbody>{visibleAttendance.map(x=><tr key={String(x.id)}><td><strong>{x.student_name}</strong></td><td>{x.record_date}</td><td><Status tone={x.status==="Hadir"?"green":x.status==="Alpa"?"red":"amber"}>{x.status}</Status></td><td>{x.note}</td><td><DataActions row={x} onEdit={r=>edit("attendance",r)} onDelete={r=>remove("attendance",r)} /></td></tr>)}{!visibleAttendance.length&&<tr><td colSpan={5} className="muted">Belum ada catatan untuk filter ini.</td></tr>}</tbody></table></div></article>
...
function PpdbApplicationModal({ onClose, onSaved, notify }: { onClose:()=>void; onSaved:()=>Promise<void>; notify:(message:string)=>void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="record-modal ppdb-form-modal" onSubmit={submit} onMouseDown={e=>e.stopPropagation()}>
    <button type="button" className="modal-close" onClick={onClose}>×</button><span className="modal-eyebrow">SPMB ONLINE 2027–2028</span><h2>Formulir calon santri</h2><p>Nomor pendaftaran dibuat otomatis. Pastikan data sesuai dokumen resmi.</p>
...
function PpdbDocumentsModal({ admission, documents, role, onClose, onUpdated, notify }: { admission:Row; documents:Row[]; role:Role; onClose:()=>void; onUpdated:()=>Promise<void>; notify:(message:string)=>void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="record-modal ppdb-documents-modal" onMouseDown={e=>e.stopPropagation()}>
    <button type="button" className="modal-close" onClick={onClose}>×</button><span className="modal-eyebrow">BERKAS SPMB</span><h2>{admission.name}</h2><p>{admission.registration_no} · PDF/JPG/PNG, maksimal 5 MB per berkas.</p>
...
function PpdbVerifyModal({ admission, onClose, onUpdated, notify }: { admission:Row; onClose:()=>void; onUpdated:()=>Promise<void>; notify:(message:string)=>void }) {
  ...
  const [wave,setWave]=useState(String(admission.wave||"Gelombang 1"));
  const [testDate,setTestDate]=useState(String(admission.test_date||""));
  const [testTime,setTestTime]=useState(String(admission.test_time||""));
  const [testLocation,setTestLocation]=useState(String(admission.test_location||""));
  const [saving,setSaving]=useState(false);
  async function save() {
    setSaving(true);
    const response=await fetch("/api/admissions",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:admission.id,status,score:Number(score),verification_note:note,wave,test_date:testDate,test_time:testTime,test_location:testLocation})});
    const result=await response.json() as {error?:string;message?:string};
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="record-modal ppdb-verify-modal" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="form-grid"><label>Status<select value={status} onChange={e=>setStatus(e.target.value)}>{ppdbStatuses.map(x=><option key={x}>{x}</option>)}</select></label><label>Nilai tes<input type="number" min="0" max="100" value={score} onChange={e=>setScore(e.target.value)}/></label><label>Gelombang<select value={wave} onChange={e=>setWave(e.target.value)}><option>Gelombang 1</option><option>Gelombang 2</option><option>Gelombang 3</option></select></label><label>Tanggal tes<input type="date" value={testDate} onChange={e=>setTestDate(e.target.value)}/></label><label>Jam tes<input type="time" value={testTime} onChange={e=>setTestTime(e.target.value)}/></label><label>Lokasi tes<input value={testLocation} onChange={e=>setTestLocation(e.target.value)} placeholder="Aula Pesantren"/></label><label className="wide">Catatan verifikator<textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Catatan yang dapat dilihat calon wali santri"/></label></div>
...
function AdmissionsPage({ data, role, reload, notify, remove }: { data:AppData; role:Role; reload:()=>Promise<void>; notify:(message:string)=>void; remove:(r:Resource,row:Row)=>void }) {
  ...
  const rankById=new Map(rows.filter(x=>Number(x.score)>0).sort((a,b)=>Number(b.score)-Number(a.score)).map((x,index)=>[String(x.id),index+1]));
  ...
  if(role==="Wali Santri") return <div className="feature-app admissions-app"><section className="summary-banner admission-banner"><div><span>SPMB ONLINE 2027–2028</span><strong>{rows.length?`${rows.length} Pendaftaran`:"Masa depan dimulai di sini."}</strong><p>Isi formulir, unggah dokumen, dan pantau hasil verifikasi secara online.</p></div><button className="light-button" onClick={()=>setShowForm(true)}>＋ Formulir Baru</button></section>
...
function GuardianAccessAdmin({ data, reload, notify }: { data:AppData; reload:()=>Promise<void>; notify:(message:string)=>void }) {
  ...
  <section className="card data-card guardian-access-table"><header className="card-header"><div><h3>Akses Portal Wali</h3><p>Periksa akun yang menunggu, lalu setujui atau blokir aksesnya</p></div><Status tone="blue">{guardians.length} wali</Status></header><div className="table-wrap"><table><thead><tr><th>Wali</th><th>Santri Terhubung</th><th>Status Akses</th><th>Status PIN</th><th /></tr></thead><tbody>{guardians.map(guardian=>{const account=data.guardianAccounts.find(row=>String(row.phone)===guardian.phone);const status=String(account?.status||"Belum mendaftar");const pinStatus=account?"Sudah diatur":"Belum dibuat";return <tr key={guardian.phone}><td><strong>{guardian.name}</strong><small className="cell-note">+{guardian.phone}</small></td><td>{guardian.students.join(", ")}</td><td><Status tone={status==="Aktif"?"green":status==="Diblokir"?"red":"amber"}>{status}</Status></td><td><Status tone={account?"green":"amber"}>{pinStatus}</Status>{account?.updated_at&&<small className="cell-note">Diperbarui {new Date(String(account.updated_at)).toLocaleDateString("id-ID")}</small>}</td><td><div className="guardian-access-actions"><button className="text-button" onClick={()=>{setSelected({phone:guardian.phone,name:guardian.name,students:guardian.students.join(", ")});setPin("");}}>{account?"Reset PIN":"Buat PIN"}</button>{account&&<button className="text-button" onClick={()=>void toggle(guardian.phone,status)}>{status==="Aktif"?"Blokir":status==="Menunggu Persetujuan"?"Setujui":"Aktifkan"}</button>}</div></td></tr>})}{!guardians.length&&<tr><td colSpan={5} className="muted">Isi nomor WhatsApp pada Data Santri terlebih dahulu.</td></tr>}</tbody></table></div></section>
...
function IntegrationsPage({ data, onImported, notify }: { data:AppData; onImported:()=>Promise<void>; notify:(s:string)=>void }) {
  ...
  async function cleanDemo(){if(!window.confirm("Data demo santri dan catatan terkait akan dihapus. Akun admin tetap dipertahankan. Lanjutkan?")) return; if(window.prompt("Ketik HAPUS DATA DEMO untuk melanjutkan")!=="HAPUS DATA DEMO"){notify("Pembersihan dibatalkan.");return;} setCleaning(true); const response=await fetch("/api/cleanup-demo",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({confirm:"HAPUS DATA DEMO"})}); const result=await response.json() as {error?:string;message?:string}; setCleaning(false); if(!response.ok){notify(result.error||"Pembersihan gagal.");return;} notify(result.message||"Data demo dibersihkan."); await onImported();}
...
function ReportsPage({ role }: { role:Role }) {
  ...
  const allReports=[
    ...
    {key:"admissions",title:"Laporan SPMB",copy:"Pendaftar dan status verifikasi",tone:"violet",admin:true},
  ];
...
const formFields: Record<Resource, { key: string; label: string; type?: string; options?: string[] }[]> = {
  ...
  admissions: [
    {key:"registration_no",label:"Nomor pendaftaran"},{key:"name",label:"Nama calon santri"},{key:"guardian_name",label:"Nama wali"},{key:"guardian_phone",label:"WhatsApp wali",type:"tel"},{key:"previous_school",label:"Asal sekolah"},{key:"status",label:"Tahap",options:ppdbStatuses},{key:"score",label:"Nilai tes",type:"number"},{key:"wave",label:"Gelombang",options:["Gelombang 1","Gelombang 2","Gelombang 3"]},{key:"test_date",label:"Tanggal tes",type:"date"},{key:"test_time",label:"Jam tes",type:"time"},{key:"test_location",label:"Lokasi tes"},
  ],
  counseling: [
...
function SettingsModal({ dark, role, onDarkChange, onHelp, notify, onClose }: { dark:boolean; role:Role; onDarkChange:(value:boolean)=>void; onHelp:()=>void; notify:(message:string)=>void; onClose:()=>void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="record-modal settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-modal-title" onMouseDown={event=>event.stopPropagation()}>
    ...
    {role==="Admin"&&<section className="branding-settings"><div className="branding-settings-head"><BrandMark className="brand-mark"/><div><strong>Logo sekolah</strong><small>Tampil di dashboard, login, Portal Wali, SPMB, kartu, dan laporan cetak.</small></div></div><label>Pilih logo baru<input type="file" accept=".png,.jpg,.jpeg,.webp" onChange={event=>setLogo(event.target.files?.[0]||null)}/><small>PNG, JPG, atau WebP · maksimal 2 MB · disarankan gambar persegi.</small></label>{logoError&&<div className="form-error">{logoError}</div>}<div className="branding-settings-actions"><button type="button" className="primary-button" disabled={savingLogo||!logo} onClick={()=>void uploadLogo()}>{savingLogo?"Menyimpan…":"Unggah Logo"}</button><button type="button" className="secondary-button" disabled={savingLogo} onClick={()=>void resetLogo()}>Gunakan Logo Bawaan</button></div></section>}
  ...
  return (
    <div className={`app-shell ${dark ? "dark" : ""}`}>
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="brand"><button className="brand-home" aria-label={role==="Wali Santri"?"Kembali ke Portal Wali":"Kembali ke dashboard"} onClick={()=>selectPage(role==="Wali Santri"?"portalwali":"dashboard")}><BrandMark className="brand-mark"/><span><strong>SINURMAN</strong><small>Sistem Informasi Nurul Iman</small></span></button><button className="close-sidebar" onClick={()=>setSidebarOpen(false)}>×</button></div>
        <nav>{visibleNavGroups.map(group=><div className="nav-group" key={group.label}><span className="nav-label">{group.label}</span>{group.items.map(item=><button key={item.key} className={page===item.key?"active":""} onClick={()=>selectPage(item.key)}><ToolIcon name={item.icon} /><span>{item.label}</span>{item.key==="pengumuman"&&data.announcements.length>0&&<b>{data.announcements.length}</b>}{item.key==="penerimaan"&&data.admissions.filter(row=>!["Lulus","Tidak Lulus"].includes(String(row.status))).length>0&&<b>{data.admissions.filter(row=>!["Lulus","Tidak Lulus"].includes(String(row.status))).length}</b>}{item.key==="portalwali"&&data.guardianMessages.filter(row=>row.status==="Baru").length>0&&<b>{data.guardianMessages.filter(row=>row.status==="Baru").length}</b>}</button>)}</div>)}</nav>
