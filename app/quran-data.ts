export type QuranSurah = { number: number; name: string; verses: number };
...
export const QURAN_SURAHS: QuranSurah[] = [
  ...
  [109,"Al-Kafirun",6],[110,"An-Nasr",3],[111,"Al-Lahab",5],[112,"Al-Ikhlas",4],
  [113,"Al-Falaq",5],[114,"An-Nas",6],
].map(([number,name,verses]) => ({ number: Number(number), name: String(name), verses: Number(verses) }));

export function quranRangeAmount(surahFrom:string,verseFrom:number,surahTo:string,verseTo:number) {
  const start=QURAN_SURAHS.findIndex(item=>item.name===surahFrom);
  const end=QURAN_SURAHS.findIndex(item=>item.name===surahTo);
  if(start<0||end<0) throw new Error("Surat tidak ditemukan pada master Al-Qur’an.");
  if(end<start) throw new Error("Surat akhir tidak boleh mendahului surat awal.");
  if(!Number.isInteger(verseFrom)||verseFrom<1||verseFrom>QURAN_SURAHS[start].verses) throw new Error(`Ayat awal ${surahFrom} harus 1–${QURAN_SURAHS[start].verses}.`);
  if(!Number.isInteger(verseTo)||verseTo<1||verseTo>QURAN_SURAHS[end].verses) throw new Error(`Ayat akhir ${surahTo} harus 1–${QURAN_SURAHS[end].verses}.`);
  if(start===end) {
    if(verseTo<verseFrom) throw new Error("Ayat akhir tidak boleh lebih kecil dari ayat awal.");
    return verseTo-verseFrom+1;
  }
  let amount=QURAN_SURAHS[start].verses-verseFrom+1+verseTo;
  for(let index=start+1;index<end;index++) amount+=QURAN_SURAHS[index].verses;
  return amount;
}
