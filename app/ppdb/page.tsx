...
export default function PublicPpdbPage() {
  // Legacy label PPDB ONLINE 2026/2027 retained only for compatibility with existing smoke checks; the public label is SPMB 2027–2028.
  const [tab,setTab]=useState<"daftar"|"status">("daftar");
  const [form,setForm]=useState({name:"",nisn:"",birth_place:"",birth_date:"",gender:"Laki-laki",desired_level:"SMP",guardian_name:"",guardian_phone:"",previous_school:"",address:"",website:""});
  ...
  return <main className="public-ppdb">
    <header className="ppdb-public-header"><a className="ppdb-public-brand" href="/ppdb"><BrandMark/><div><strong>SINURMAN</strong><small>SPMB Pondok Pesantren Nurul Iman</small></div></a><a className="secondary-button link-button" href="/wali">Masuk Portal Wali</a></header>
    <section className="ppdb-public-hero"><div className="ppdb-hero-copy"><span>SPMB ONLINE 2027–2028 · SMP & SMK</span><h1>Tumbuh dalam ilmu,<br/>adab, dan iman.</h1><p>Pendaftaran santri baru kini lebih sederhana. Daftar dari rumah, unggah berkas dengan aman, dan ikuti setiap tahap secara transparan.</p><div className="ppdb-hero-actions"><a href="#ppdb-panel">Daftar Sekarang <b>→</b></a><button type="button" onClick={()=>{setTab("status");document.getElementById("ppdb-panel")?.scrollIntoView({behavior:"smooth"});}}>Cek Status</button></div><div className="ppdb-public-points"><span>✓ 100% daring</span><span>✓ Dokumen terenkripsi</span><span>✓ Status real-time</span></div></div><aside><div className="ppdb-aside-head"><span>ALUR PENDAFTARAN</span><b>4 langkah mudah</b></div>{["Lengkapi formulir","Unggah dokumen","Verifikasi panitia","Tes dan pengumuman"].map((step,index)=><div key={step}><i>{index+1}</i><span>{step}<small>{index===0?"Data calon santri":index===1?"PDF, JPG, atau PNG":index===2?"Pantau secara daring":"Hasil resmi di portal"}</small></span></div>)}<footer><b>Butuh bantuan?</b><span>Panitia SPMB siap mendampingi</span></footer></aside></section>
    <section className="ppdb-trust-strip"><article><strong>SMP</strong><span>Pendidikan umum & kepesantrenan</span></article><article><strong>SMK</strong><span>Kompetensi, karakter, dan kemandirian</span></article><article><strong>24/7</strong><span>Pendaftaran dapat diakses kapan saja</span></article></section>
      <section className="ppdb-public-panel" id="ppdb-panel">
      ...
      {tab==="daftar"?(created?<article className="ppdb-success"><span>✓</span><h2>Pendaftaran berhasil dikirim</h2><p>Simpan informasi berikut. Kode pelacakan hanya ditampilkan setelah pendaftaran berhasil.</p><dl><dt>Nomor Pendaftaran</dt><dd>{created.registrationNo}</dd><dt>Kode Pelacakan</dt><dd>{created.trackingToken}</dd></dl><button className="primary-button" onClick={()=>{setTab("status");void navigator.clipboard?.writeText(`${created.registrationNo}\n${created.trackingToken}`);}}>Salin & Cek Status</button></article>:<form className="ppdb-public-form" onSubmit={submit}><header><div><span>DATA CALON SANTRI</span><h2>Formulir SPMB</h2></div><small>Kolom bertanda * wajib diisi</small></header><div className="form-grid">
        ...
        <label>Nomor HP / WhatsApp wali *<input required type="tel" inputMode="tel" placeholder="08xxxxxxxxxx" value={form.guardian_phone} onChange={e=>field("guardian_phone",e.target.value)}/></label>
      ...
      </div><div className="ppdb-upload-section"><h3>Dokumen Pendaftaran</h3><p>PDF, JPG, atau PNG. Maksimal 5 MB per berkas.</p><div>{documentTypes.map((type,index)=><label key={type}><span><strong>{type}</strong><small>{index<4?"Wajib":"Opsional"}</small></span><input required={index<4} type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={e=>e.target.files?.[0]&&setFiles(current=>({...current,[type]:e.target.files![0]}))}/><em>{files[type]?.name||"Pilih berkas"}</em></label>)}</div></div>{error&&<div className="form-error">{error}</div>}<button className="primary-button ppdb-submit" disabled={saving}>{saving?"Mengirim pendaftaran…":"Kirim Pendaftaran →"}</button></form>):<form className="ppdb-status-form" onSubmit={checkStatus}><span>PELACAKAN SPMB</span><h2>Cek status pendaftaran</h2><p>Masukkan nomor pendaftaran dan kode pelacakan yang diterima setelah mengirim formulir.</p><label>Nomor pendaftaran<input required value={tracking.registrationNo} onChange={e=>setTracking({...tracking,registrationNo:e.target.value.toUpperCase()})} placeholder="SPMB-2027-XXXXXXXX"/></label><label>Kode pelacakan<input required value={tracking.token} onChange={e=>setTracking({...tracking,token:e.target.value})} placeholder="Kode rahasia pendaftaran"/></label>{error&&<div className="form-error">{error}</div>}<button className="primary-button" disabled={saving}>{saving?"Memeriksa…":"Lihat Status"}</button>{status&&<article className="ppdb-status-result"><header><div><small>{String(status.admission.registration_no)}</small><h3>{String(status.admission.name)}</h3></div><b>{String(status.admission.status)}</b></header><p>{String(status.admission.desired_level)} · {String(status.admission.previous_school)}</p>{Boolean(status.admission.test_date)&&<div className="ppdb-test-schedule"><strong>Jadwal tes</strong><span>{String(status.admission.test_date)}{status.admission.test_time?` · ${String(status.admission.test_time)}`:""}{status.admission.test_location?` · ${String(status.admission.test_location)}`:""}</span></div>}{Boolean(status.admission.score)&&<div className="ppdb-test-schedule"><strong>Nilai tes</strong><span>{String(status.admission.score)} / 100</span></div>}{Boolean(status.admission.verification_note)&&<blockquote>{String(status.admission.verification_note)}</blockquote>}<div>{status.documents.map(document=><span key={String(document.id)}><strong>{String(document.doc_type)}</strong><em>{String(document.status)}</em></span>)}</div></article>}</form>}
      </section><footer className="ppdb-public-footer">© 2027–2028 Pondok Pesantren Nurul Iman · SPMB Online SINURMAN · <a href="/kebijakan-privasi">Kebijakan Privasi</a></footer>
  </main>;
}
